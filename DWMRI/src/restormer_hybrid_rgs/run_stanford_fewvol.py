...
Usage (from ``DWMRI/src`` with PYTHONPATH)::
    ...
    python -m restormer_hybrid_rgs.run_stanford_fewvol
    python -m restormer_hybrid_rgs.run_stanford_fewvol --force-train
    python -m restormer_hybrid_rgs.run_stanford_fewvol --skip-train
"""
...
def _build_checkpoint_dir(
    ...
    *,
    ...
    if rgs and g_shell is not None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Restormer RGS: train on few Stanford DWI volumes; reconstruct all non-b0 DWI."
    )
    ...
    if use_rgs:
        settings.model.in_channel = k_in
    else:
    ...
    logging.info(
        f"Stanford few-volume (Restormer RGS): train_num_volumes={train_num_volumes}, "
        f"reconstruct_full_dwi={reconstruct_full}, rgs={use_rgs}"
    )
    logging.info(
    ...
    if not args.no_wandb:
        ...
        wandb_run = wandb.init(
            ...
            name=f"restormer_stanford_fewvol_{bvalue_segment}_nvol{train_num_volumes}_{noise_segment}",
            tags=["stanford", "fewvol", "restormer_rgs", bvalue_segment, noise_segment],
            config=wandb_config,
        )
...
def _run_stanford_fewvol_body(
    ...
    rec_model = _make_model(settings, inp_ch)
    ...
    if reconstruct_full:
        if use_rgs:
            ...
            rec_vxyz = reconstruct_dwis_rgs(
                ...
                n_context=int(getattr(settings.reconstruct, "n_context_samples", 10)),
