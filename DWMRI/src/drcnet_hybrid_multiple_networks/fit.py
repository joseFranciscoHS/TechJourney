...
def fit_transfer_wrapper(
    ...
    """
    Train only the adapter layers of DenoiserNetTransferWrapper (base is frozen).
    Same loss and dataloader format as fit_model (masked input + noisy target).
    """
    logging.info(
        f"Starting transfer wrapper training - device: {device}, epochs: {num_epochs}"
    )
    ...
    logging.info(f"Transfer training starting from epoch: {start_epoch}")
    logging.info(f"Best loss so far: {best_loss:.6f}")
    ...
    for epoch in tqdm(
        ...
        desc="Training transfer wrapper",
        total=num_epochs,
    ):
        model.train()
        total_loss = 0
        ...
        for batch_idx, (x, mask, noisy_target_volume) in enumerate(train_loader):
            ...
            if batch_idx % 10 == 0:
                logging.debug(
                    ...
                    f"noisy_target_volume shape: {noisy_target_volume.shape}"
                )
        ...
        logging.info(f"Average Loss: {avg_loss:.6f}")
        logging.info(
            f"Loss stats - Min: {min_loss:.6f}, Max: {max_loss:.6f}, Std: {std_loss:.6f}"
        )
        logging.info(f"Learning rate: {current_lr:.6f} -> {new_lr:.6f}")
        ...
        if wandb.run is not None:
            wandb.log(
                {
                    "epoch": epoch + 1,
                    "transfer/train_loss": avg_loss,
                    "transfer/loss_min": min_loss,
                    "transfer/loss_max": max_loss,
                    "transfer/loss_std": std_loss,
                    "transfer/learning_rate": new_lr,
                    "transfer/best_loss": best_loss,
                }
            )

    logging.info("Transfer wrapper training completed successfully.")
    logging.info(f"Final best loss: {best_loss:.6f}")
    logging.info(f"Total epochs trained: {num_epochs - start_epoch}")
