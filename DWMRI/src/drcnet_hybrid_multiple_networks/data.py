    ...
    for x in range(0, image.shape[1] - patch_size[1] + 1, step):
        for y in range(0, image.shape[2] - patch_size[2] + 1, step):
            for z in range(0, image.shape[3] - patch_size[3] + 1, step):
                # Check if patch should be included based on filter method
                include_patch = True
...
class TrainingDataSet(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            source_volume_index: target volume index (if None, cycles through all)
        ...
        self.source_volume_index = source_volume_index
    ...
    def set_source_volume_index(self, source_volume_index: int):
        self.source_volume_index = source_volume_index

    def __getitem__(self, index: int):
        # When source_volume_index is set, one sample per window; else windows × volumes
        window_idx = (
            index if self.source_volume_index is not None else index // self.n_volumes
        )
        # we are going to train the model first on a particular volume
        # according to the J-invariance, the network should learn to
        # denoise a unknown target volume given the source volumes
        # in this case we are going to train the model
        # and latter apply transfer learning to the rest of the volumes
        target_volume_idx = self.source_volume_index
    ...
    def __len__(self):
        ...
        length = (
            len(self.windows)
            if self.source_volume_index is not None
