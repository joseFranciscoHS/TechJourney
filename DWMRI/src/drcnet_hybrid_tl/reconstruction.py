...
def reconstruct_single_volume(
    model, data, vol_idx, device, mask_p=0.3, n_preds=10
):
    """
    Reconstruct one target volume using the given model (base or transfer wrapper).
    Same masking and averaging as reconstruct_dwis, but for a single volume index.

    Args:
        model: DenoiserNet or DenoiserNetTransferWrapper
        data: (Vols, X, Y, Z) tensor
        vol_idx: target volume index
        device: device
        mask_p: mask probability
        n_preds: number of predictions to average

    Returns:
        Reconstructed volume shape (X, Y, Z)
    """
    model.to(device)
    model.eval()
    num_vols, x_size, y_size, z_size = data.shape
    spatial_dims = (x_size, y_size, z_size)
    sum_pred = np.zeros(spatial_dims, dtype=np.float32)
    ...
    with torch.inference_mode():
        ...
        for pred_idx in range(n_preds):
            ...
            reconstructed = model(data_masked)
            pred_volume = (
                reconstructed.squeeze(0).squeeze(0).detach().cpu().numpy()
            )
            sum_pred += pred_volume

    return sum_pred / n_preds
