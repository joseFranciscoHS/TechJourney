...
def detect_gpu_imbalance(
    gpu_ids: List[int], memory_threshold: float = 0.75
    ...
    for gpu in gpu_props[1:]:
        ...
        if memory_ratio < memory_threshold or cores_ratio < memory_threshold:
            ...
            logging.warning(
                f"  Memory: {gpu['memory'] / (1024**3):.1f}GB vs {ref_gpu['memory'] / (1024**3):.1f}GB (ratio: {memory_ratio:.2f})"
            )
            logging.warning(
                f"  Cores: {gpu['cores']} vs {ref_gpu['cores']} (ratio: {cores_ratio:.2f})"
            )
        else:
...
def setup_multi_gpu(
    ...
    if available_gpus < requested_gpus:
        if verbose:
            logging.warning(
                f"Only {available_gpus} GPUs available, requested {requested_gpus}"
            )
    ...
    if config.auto_exclude_imbalanced and len(gpu_ids) > 1:
        balanced_gpus, imbalanced_gpus = detect_gpu_imbalance(
            gpu_ids, config.memory_threshold
        )
    ...
    if len(gpu_ids) > 1:
        ...
        if config.auto_scale_lr:
            ...
            if verbose:
                logging.info(
                    f"Learning rate scaled: {config.base_learning_rate} -> {effective_lr:.6f} (factor: {scale_factor:.2f})"
                )
                logging.info(
                    f"Effective batch size: {config.batch_size_per_gpu} -> {effective_batch_size} (per GPU × {len(gpu_ids)} GPUs)"
                )
            return model, effective_lr, effective_batch_size
        else:
            if verbose:
                logging.info(
                    f"Learning rate scaling disabled, using base LR: {config.base_learning_rate}"
                )
        ...
        if verbose:
            logging.info(
                "Single GPU training (multi_gpu enabled but only 1 suitable GPU available)"
            )
        return model, config.base_learning_rate, config.batch_size_per_gpu
...
def get_gpu_info() -> dict:
    ...
    gpu_info = {"count": gpu_count, "devices": []}
...
def validate_multi_gpu_setup(config: MultiGPUConfig) -> bool:
    ...
    if invalid_gpu_ids:
        logging.error(
            f"Invalid GPU IDs: {invalid_gpu_ids}. Available GPUs: 0-{available_gpus - 1}"
        )
        return False
...
def create_single_gpu_config(
    base_lr: float = 0.0005, batch_size: int = 16
) -> MultiGPUConfig:
