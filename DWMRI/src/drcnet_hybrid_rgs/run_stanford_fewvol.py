...
``input_channels`` matches ``train_num_volumes`` (sequential) or ``num_input_volumes`` (RGS).
Full-dataset inference: :func:`reconstruct_full_dwi_chunked` (sequential) or
:func:`reconstruct_dwis_rgs` when ``shell_sampling_mode: rgs``.

Usage (from ``DWMRI/src`` with PYTHONPATH)::

    python -m drcnet_hybrid_rgs.run_stanford_fewvol
    python -m drcnet_hybrid_rgs.run_stanford_fewvol --force-train
    python -m drcnet_hybrid_rgs.run_stanford_fewvol --skip-train
"""
...
def _run_stanford_fewvol_body(
    ...
    if do_train:
        ...
        model = _make_denoiser(
            settings, k_in if use_rgs else train_num_volumes, spatial
        )
    ...
    rec_model = _make_denoiser(
        settings, k_in if use_rgs else train_num_volumes, spatial_full
    )
    ...
    if reconstruct_full:
        if use_rgs:
            logging.info(
                f"RGS reconstruct: all {full_noisy.shape[-1]} DWI volumes "
