...
def main(
    ...
    logging.info(f"Mixed precision: {'Enabled' if use_mixed_precision else 'Disabled'}")
    ...
    logging.info(
        f"Data type: {original_data.dtype}, Min: {original_data.min():.4f}, Max: {original_data.max():.4f}, Mean: {original_data.mean():.4f}"
    )

    train_set = TrainingDataSetMultipleVolumes(
        noisy_data,
        patch_size=(
