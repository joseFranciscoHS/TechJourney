...
from restormer_hybrid.reconstruction import reconstruct_dwis
...
def fit_progressive(
    ...
    for i, stage in enumerate(stages):
        logging.info(
            f"  Stage {i+1}: patch={stage.patch_size}³, batch={stage.batch_size}, epochs={stage.epochs}, step={stage.step}"
        )
...
def main(
    ...
    logging.info(f"Starting Restormer3D training with dataset: {dataset}")
