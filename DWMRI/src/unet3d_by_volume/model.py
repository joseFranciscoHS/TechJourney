"""
3D U-Net for DWMRI by-volume denoising.

By-volume paradigm: X-1 noisy volumes as input, predict the denoised jth volume
(the one excluded from input). For X total volumes, in_channel = X - 1.

Expected input shape: (B, X-1, D, H, W)
Output shape: (B, 1, D, H, W) - denoised jth volume
"""
...
class Unet3D(nn.Module):
    ...
    Args:
        in_channel: Number of input channels = num_volumes - 1 (X-1 volumes)
