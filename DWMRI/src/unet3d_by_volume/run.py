...
import numpy as np
import torch
from unet3d_by_volume.data import ReconstructionDataSet, TrainingDataSet
from unet3d_by_volume.fit import fit_model
from unet3d_by_volume.model import Unet3D
from unet3d_by_volume.reconstruction import reconstruct_dwis
...
def main(
    ...
    try:
        ...
        in_channel = settings.data.num_volumes - 1
        model = Unet3D(
        ...
        logging.info(
            f"Model initialized - in_channel: {in_channel} (num_volumes-1), out_channel: {settings.model.out_channel}"
        )
        logging.info(
