...
:func:`drcnet_hybrid_2.reconstruction.reconstruct_full_dwi_chunked`, which calls
``reconstruct_dwis`` on contiguous volume blocks and concatenates.

Usage (from ``DWMRI/src`` with PYTHONPATH)::

    python -m drcnet_hybrid_2.run_stanford_fewvol
    python -m drcnet_hybrid_2.run_stanford_fewvol --force-train
    python -m drcnet_hybrid_2.run_stanford_fewvol --skip-train
"""
...
from drcnet_hybrid_2.run import fit_progressive
