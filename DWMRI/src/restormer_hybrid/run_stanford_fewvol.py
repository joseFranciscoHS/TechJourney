"""
Stanford HARDI: train on a small number of DWI volumes, reconstruct all non-b0 volumes.

The Restormer expects ``inp_channels ==`` the number of volumes in each patch. Training
uses ``train_num_volumes`` (e.g. 10). Full-dataset inference splits all DWI volumes
(after b0s) into contiguous chunks of that size and runs ``reconstruct_dwis`` per
chunk; results are concatenated (see ``reconstruct_full_dwi_chunked``).

Usage (from ``DWMRI/src`` with PYTHONPATH set)::
...
from restormer_hybrid.reconstruction import (
    reconstruct_dwis,
    reconstruct_full_dwi_chunked,
)
from restormer_hybrid.run import fit_progressive
...
def _prepare_stanford_arrays(settings, train_num_volumes: int):
    """
    Spatial crop, then split into (train stack) and (all DWI after b0s).

    Returns:
        train_noisy, train_orig: (X,Y,Z, train_num_volumes)
        full_noisy, full_orig: (X,Y,Z, V_all_dwi) — all non-b0 volumes in the crop
    """
    d = settings.data
...
def _make_model(settings, inp_channels: int) -> Restormer3D:
    return Restormer3D(
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Restormer on few Stanford DWI volumes; reconstruct all non-b0 DWI."
    )
    parser.add_argument(
        "--force-train",
        action="store_true",
        help="Train even if a checkpoint already exists.",
    )
    parser.add_argument(
        "--skip-train",
        action="store_true",
        help="Only reconstruct (requires existing checkpoint).",
    )
    parser.add_argument(
        "--skip-reconstruct",
        action="store_true",
        help="Only train; skip reconstruction and metrics.",
    )
    parser.add_argument(
        "--no-wandb",
        action="store_true",
    ...
    logging.info(
        f"Stanford few-volume experiment: train_num_volumes={train_num_volumes}, "
    ...
    if not args.no_wandb:
        ...
        wandb_run = wandb.init(
            ...
            name=f"stanford_fewvol_{bvalue_segment}_nvol{train_num_volumes}_{noise_segment}",
            tags=["stanford", "fewvol", bvalue_segment, noise_segment],
            config=wandb_config,
        )
...
def _run_stanford_fewvol_body(
    ...
    if do_train:
        ...
        model = _make_model(settings, train_num_volumes)
    ...
    logging.info("Loading checkpoint for reconstruction...")
    rec_dev = settings.reconstruct.device
    use_amp_rec = getattr(settings.reconstruct, "use_amp", True)
    rec_model = _make_model(settings, train_num_volumes)
    ...
    psize = getattr(settings.reconstruct, "patch_size", 32)
    ...
    if reconstruct_full:
        ...
        logging.info("Reconstructing training subset only (same V as training).")
